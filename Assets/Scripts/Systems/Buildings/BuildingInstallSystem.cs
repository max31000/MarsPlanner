...
namespace Systems.Buildings
{
    public class BuildingInstallSystem : IEcsRunSystem
    {
        ...
        private void InstantiateNewBuilding(ref PlaceBuildProcessingComponent buildingPlaceComponent)
        {
            ...
            FlatTerrainUnderBuilding(buildingPlaceComponent.Position,
                installedBuild.GetComponentsInChildren<Collider>().First().bounds.size, 0.6f);

            var size = buildingsBufferPool.Value.Get(buildingsBufferFilter.Value.Single())
                .BuildingsBuffer[buildingPlaceComponent.Type].StartBoundSize;
            var rotationY = installedBuild.transform.eulerAngles.y;
            var navigationSystem = navigationDataFilter.Pools.Inc1.Get(navigationDataFilter.Value.Single())
                .NavigationSystem;
            var nodes = navigationSystem
                .GetNodesInArea(installedBuild.transform.position, (int)size.x, (int)size.z, rotationY);

            foreach (var node in nodes)
            {
                node.Status = NodeStatus.Disabled;
                Debug.DrawLine(node.Position, node.Position + Vector3.up * 5, Color.black, 10000);
            }

            var localBuildNavigation = installedBuildComponent.Object.GetComponentInChildren<LocalNavigation>();
            var transform = localBuildNavigation.transform;
            var localNavigationGraph = localBuildNavigation.LocalGraph.ToUndirectedGraph(transform.position, transform.rotation.eulerAngles.y);
            var outNodes = localNavigationGraph.Vertices.Where(x => x.Type == NodeType.BuildOutNode).ToArray();

            foreach (var outNode in outNodes)
            {
                var closestMainNode = navigationSystem.FindClosestNode(outNode.Position, NodeType.GridNode, NodeStatus.Enabled);
                navigationSystem.AddSubgraph(localNavigationGraph, outNode, closestMainNode);
                Debug.DrawLine(outNode.Position, closestMainNode.Position, Color.cyan, 10000, false);
            }
        }
        ...
        private static float FindCenterMin(float[,] array)
        {
            ...
            var startRow = rows / 2 - 1 - (rows % 2 == 0 ? 1 : 0);
            var endRow = rows / 2 + 1;
            var startCol = cols / 2 - 1 - (cols % 2 == 0 ? 1 : 0);
            var endCol = cols / 2 + 1;
